...
# The public and private keys for this program are created
# by the makePublicPrivateKeys.py program. This program
# must be run in the same folder as the key files.
...
def encryptMessage(message, key, blockSize):
    ...
    for block in getBlocksFromText(message, blockSize):
        ...
        encryptedBlocks.append(pow(block, e , n))  # pow(x,y) is x**y and pow(x,y,z) is x**y % z
    return encryptedBlocks


def decryptMessage(encryptedBlocks, messageLength, key, blockSize):
    # Decrypts a list of encrypted block integers into the original message
...
# If publicKeyCipher.py is run (instead of imported as a module), call the main() function
